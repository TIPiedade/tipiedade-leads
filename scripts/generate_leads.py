...
def corpo_html(email_num, grupo, nome_lead, zona, nome_comercial, texto_plain):
    ...
    cores = {1:"#5C2D0E", 2:"#D4682A", 3:"#C49A3C", 4:"#5C2D0E"}
    cor_barra = cores.get(email_num, "#5C2D0E")
    ...
    for linha in texto_plain.strip().split("\n"):
        ...
        if not l:
            ...
            parags.append(f'<p style="margin:4px 0 4px 16px;color:#2C1A0A"><span style="color:#C49A3C;font-weight:700">✦</span> {l[1:].strip()}</p>')
        elif l.startswith("→"):
            parags.append(f'<p style="margin:4px 0 4px 16px;color:#2C1A0A"><span style="color:#D4682A;font-weight:700">→</span> {l[1:].strip()}</p>')
        elif any(l.startswith(x) for x in ["Rui Bernardes","Departamento","sales@","www."]):
            parags.append(f'<p style="margin:1px 0;color:#6B5744;font-size:12px">{l}</p>')
        elif l.startswith("Com os melhores"):
            parags.append(f'<p style="margin:12px 0 4px;color:#2C1A0A">{l}</p>')
        else:
            parags.append(f'<p style="margin:8px 0;color:#2C1A0A;line-height:1.65">{l}</p>')
    paragrafos = "".join(parags)

    html_parts = [
        '''<!DOCTYPE html><html lang="pt"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
</head><body style="margin:0;padding:0;background:#F8F0E3;font-family:Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" style="background:#F8F0E3;padding:24px 0">
<tr><td align="center">
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%">
  <tr>
    <td style="background:#4a2417;border-radius:14px 14px 0 0;padding:24px 36px 18px;text-align:center">''',
        f'<img src="{LOGO_WHITE}" alt="Ti\'Piedade" style="height:60px;display:block;margin:0 auto;border:0">',
        '<div style="height:2px;background:linear-gradient(90deg,transparent,#C49A3C,transparent);margin-top:16px;border-radius:2px"></div></td></tr>',
        '<tr><td style="background:#ffffff;padding:32px 40px 24px;border-left:1px solid #E8D5B8;border-right:1px solid #E8D5B8">',
        paragrafos,
        '</td></tr>',
        f'<tr><td style="background:{cor_barra};height:3px;border-left:1px solid #E8D5B8;border-right:1px solid #E8D5B8"></td></tr>',
        '''<tr><td style="background:#FDF6EF;padding:18px 40px;border:1px solid #E8D5B8;border-top:none">
  <p style="margin:0;font-size:11px;font-weight:700;color:#C49A3C;letter-spacing:.08em;text-transform:uppercase">O produto</p>
  <p style="margin:5px 0 0;font-size:14px;font-weight:700;color:#4a2417">Pão de Ló Ti\'Piedade — Unidose 85g (congelado)</p>
  <p style="margin:3px 0 0;font-size:12px;color:#6B5744">Original · Chocolate · Canela · Café &nbsp;|&nbsp; Receita artesanal desde 1984 · Sem conservantes</p>
</td></tr>
<tr><td style="background:#4a2417;border-radius:0 0 14px 14px;padding:16px 36px">
  <table width="100%" cellpadding="0" cellspacing="0"><tr>
    <td><p style="margin:0;font-size:12px;color:#C49A3C;font-weight:700">Pão de Ló Ti\'Piedade</p>
        <p style="margin:2px 0 0;font-size:11px;color:rgba(255,255,255,.55)">sales@tipiedade.com · www.tipiedade.com</p></td>
    <td align="right"><a href="https://www.tipiedade.com" style="font-size:11px;font-weight:700;color:#C49A3C;text-decoration:none;border:1px solid rgba(196,154,60,.5);padding:6px 14px;border-radius:6px">Ver produto ↗</a></td>
  </tr></table>
</td></tr>
