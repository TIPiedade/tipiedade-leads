...
def corpo_html(email_num, grupo, nome_lead, zona, nome_comercial, texto_plain):
    """Gera o email HTML com design Ti\'Piedade — ícones SVG vintage, história, textos afáveis."""
    ...
    # ── Intros por tipologia ──────────────────────────────────────
    INTROS = {'hotel': ['identificámos o vosso hotel como um espaço que cuida da experiência dos hóspedes em cada detalhe — incluindo à mesa.', 'Na semana passada tivemos o prazer de nos apresentar. Hoje gostaríamos de partilhar algo concreto — um resultado real, de uma unidade hoteleira semelhante à vossa.', 'Há uma tendência que temos observado no setor hoteleiro e que nos parece relevante partilhar.', 'Ao longo das últimas semanas partilhámos a nossa história e alguns casos reais no canal hoteleiro.'], 'pastelaria': ['identificámos a vossa pastelaria como um espaço que valoriza o produto artesanal — e gostaríamos de vos apresentar um complemento que acreditamos ser uma boa adição ao vosso catálogo.', 'Na semana passada tivemos o prazer de nos apresentar. Hoje gostaríamos de partilhar a experiência de uma pastelaria que trabalha connosco.', 'Há algo que temos observado no mercado da pastelaria artesanal que nos parece relevante partilhar convosco.', 'Ao longo das últimas semanas partilhámos a nossa história e casos reais em pastelarias parceiras.'], 'catering': ["identificámos a vossa empresa como uma referência na organização de eventos — e gostaríamos de partilhar como o Ti'Piedade pode simplificar a vossa proposta de sobremesas.", 'Na semana passada tivemos o prazer de nos apresentar. Hoje partilhamos a experiência de uma empresa de catering que trabalha connosco.', 'Há uma tendência nos eventos premium que nos parece relevante partilhar convosco.', 'Ao longo das últimas semanas partilhámos a nossa história. Esta semana gostaríamos de passar para deixar amostras.'], 'distribuidor': ["identificámos a vossa empresa como um parceiro com cobertura regional relevante — e gostaríamos de explorar uma possível colaboração na distribuição do Ti'Piedade.", 'Na semana passada tivemos o prazer de nos apresentar. Hoje partilhamos dados concretos sobre a procura do nosso produto.', 'Há regiões com procura crescente onde ainda não temos representação — e acreditamos que a vossa empresa seria o parceiro certo.', 'Ao longo das últimas semanas partilhámos a nossa história e o potencial de mercado. Esta semana gostaríamos de apresentar as condições comerciais.'], 'gourmet': ["identificámos o vosso espaço como um lugar que cuida da narrativa de cada produto em prateleira — e acreditamos que o Ti'Piedade tem muito a oferecer nesse sentido.", 'Na semana passada tivemos o prazer de nos apresentar. Hoje partilhamos a experiência de uma mercearia gourmet que trabalha connosco.', 'Há algo que os consumidores de produto gourmet estão a valorizar cada vez mais — e que nos parece relevante partilhar convosco.', 'Ao longo das últimas semanas partilhámos a nossa história. Esta semana gostaríamos de passar para mostrar o produto pessoalmente.'], 'cafe': ["identificámos o vosso café como um espaço que cuida da experiência de cada cliente — e gostaríamos de partilhar como o Ti'Piedade pode complementar a vossa oferta.", 'Na semana passada tivemos o prazer de nos apresentar. Hoje partilhamos a experiência de um café que trabalha connosco.', 'Há uma tendência que temos observado nos cafés de especialidade que nos parece relevante partilhar.', 'Ao longo das últimas semanas partilhámos a nossa história. Esta semana gostaríamos de passar com amostras para a vossa equipa provar.'], 'cervejaria': ["identificámos a vossa cervejaria como um espaço com grande volume de refeições — e gostaríamos de partilhar como o Ti'Piedade pode tornar a vossa sobremesa mais simples e mais rentável.", 'Na semana passada tivemos o prazer de nos apresentar. Hoje partilhamos a experiência de uma cervejaria que trabalha connosco.', 'Há algo que estamos a observar no setor das cervejarias que nos parece relevante partilhar.', 'Ao longo das últimas semanas partilhámos a nossa história. Esta semana gostaríamos de passar com amostras.'], 'restaurante': ['identificámos o vosso restaurante como um espaço que valoriza a qualidade à mesa — e gostaríamos, com toda a simplicidade, de vos apresentar o nosso produto.', 'Na semana passada tivemos o prazer de nos apresentar. Hoje gostaríamos de partilhar algo concreto — um resultado real, de um espaço semelhante ao vosso.', 'Há uma tendência que temos observado no setor e que nos parece relevante partilhar — não como argumento de venda, mas como reflexão genuína sobre o que os clientes estão a valorizar.', 'Ao longo das últimas semanas partilhámos a nossa história e alguns resultados reais. Esta semana, se nos permitirem, gostaríamos de passar para deixar amostras dos quatro sabores.']}

    intros    = INTROS.get(grupo, INTROS["restaurante"])
    frase     = intros[email_num-1] if email_num <= len(intros) else intros[0]
    if frase.lower().startswith("bom dia"):
        intro = frase
    else:
        intro = (
            f"Bom dia,<br><br>"
            f"O meu nome é <strong>Rui Bernardes</strong> e faço parte da equipa comercial do "
            f"<strong>Pão de Ló Ti\'Piedade</strong>.<br><br>{frase}"
        )

    # ── CTAs ─────────────────────────────────────────────────────
    CTAS = {1: ('✉ Responder', '#5C2D0E', 'Ficamos ao dispor para qualquer questão. Será um prazer apresentar o produto pessoalmente.'), 2: ('✉ Saber mais', '#D4682A', 'Se quiser perceber como isto poderia funcionar no vosso espaço, estamos disponíveis para uma conversa sem compromisso.'), 3: ('✉ Vamos conversar?', '#5C2D0E', 'Podemos passar pela vossa zona na próxima semana, sem pressa nem compromisso.'), 4: ('✉ Agendar uma visita', '#1A6B3A', 'Sem apresentações formais, sem pressão. Só o produto, para o descobrirem por si próprios.')}
    cta_btn, cta_cor, cta_sub = CTAS.get(email_num, CTAS[1])

    # ── Benefícios com ícones SVG ─────────────────────────────────
    BENS = {'restaurante': [('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22V12"/><path d="M12 12C12 12 7 10 7 5a5 5 0 0 1 5-5 5 5 0 0 1 5 5c0 5-5 7-5 7z"/><path d="M9 15c-2 1-4 0-4 0s.5-3 2.5-4"/><path d="M15 15c2 1 4 0 4 0s-.5-3-2.5-4"/></svg>', 'Receita artesanal desde 1984', 'Ingredientes simples, sem conservantes, feito com o cuidado de quem tem 40 anos de prática'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M8 6l4-4 4 4"/><path d="M8 18l4 4 4-4"/><path d="M6 8l-4 4 4 4"/><path d="M18 8l4 4-4 4"/><path d="M9 9l3-3 3 3"/><path d="M9 15l3 3 3-3"/><path d="M15 9l3 3-3 3"/><path d="M9 9l-3 3 3 3"/></svg>', 'Dose individual congelada', 'Serve só o que precisa, quando precisa — zero desperdício, total comodidade'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M6 13.87A4 4 0 0 1 7.41 6a5.11 5.11 0 0 1 1.05-1.54 5 5 0 0 1 7.08 0A5.11 5.11 0 0 1 16.59 6 4 4 0 0 1 18 13.87V21H6Z"/><line x1="6" y1="17" x2="18" y2="17"/></svg>', 'Sem precisar de pasteleiro', 'Descongela em minutos e vai directo ao prato, mantendo toda a qualidade'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v2m0 8v2m-4-6h8"/></svg>', 'Quatro sabores à escolha', 'Original, Chocolate, Canela e Café — para combinar com qualquer carta')], 'hotel': [('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M2 19h20M4 15l4-8 4 5 4-5 4 8"/></svg>', 'Qualidade constante, em qualquer turno', 'Artesanal e consistente — do primeiro ao último hóspede do dia'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M8 6l4-4 4 4"/><path d="M8 18l4 4 4-4"/><path d="M6 8l-4 4 4 4"/><path d="M18 8l4 4-4 4"/><path d="M9 9l3-3 3 3"/><path d="M9 15l3 3 3-3"/><path d="M15 9l3 3-3 3"/><path d="M9 9l-3 3 3 3"/></svg>', 'Sem preparação, sem perdas', 'Dose individual congelada — do frigorífico ao prato em minutos'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>', 'Uma história para contar', 'Receita levada pelos portugueses ao Japão no século XVI — genuinamente portuguesa'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M17 8h1a4 4 0 0 1 0 8h-1"/><path d="M3 8h14v9a4 4 0 0 1-4 4H7a4 4 0 0 1-4-4Z"/><line x1="6" y1="2" x2="6" y2="4"/><line x1="10" y1="2" x2="10" y2="4"/><line x1="14" y1="2" x2="14" y2="4"/></svg>', 'Versátil em qualquer serviço', 'Restaurante, room service, pequeno-almoço ou eventos')], 'pastelaria': [('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22V12"/><path d="M12 12C12 12 7 10 7 5a5 5 0 0 1 5-5 5 5 0 0 1 5 5c0 5-5 7-5 7z"/><path d="M9 15c-2 1-4 0-4 0s.5-3 2.5-4"/><path d="M15 15c2 1 4 0 4 0s-.5-3-2.5-4"/></svg>', 'Complemento, não concorrente', 'Uma referência nacional que os vossos clientes já conhecem e procuram'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M8 6l4-4 4 4"/><path d="M8 18l4 4 4-4"/><path d="M6 8l-4 4 4 4"/><path d="M18 8l4 4-4 4"/><path d="M9 9l3-3 3 3"/><path d="M9 15l3 3 3-3"/><path d="M15 9l3 3-3 3"/><path d="M9 9l-3 3 3 3"/></svg>', 'Dose individual, controlo total', 'Sem perdas, sem desperdício — vai ao balcão quando é pedido'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="13" x2="15" y2="13"/><line x1="9" y1="17" x2="15" y2="17"/></svg>', 'Receita com 40 anos intacta', 'Ingredientes simples, sem conservantes, origem verificável'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v2m0 8v2m-4-6h8"/></svg>', 'Margem interessante para revenda', 'Quatro sabores: Original, Chocolate, Canela e Café')], 'catering': [('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>', 'Para qualquer dimensão de evento', 'De 20 a 2000 convidados, com a mesma qualidade e simplicidade'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M8 6l4-4 4 4"/><path d="M8 18l4 4 4-4"/><path d="M6 8l-4 4 4 4"/><path d="M18 8l4 4-4 4"/><path d="M9 9l3-3 3 3"/><path d="M9 15l3 3 3-3"/><path d="M15 9l3 3-3 3"/><path d="M9 9l-3 3 3 3"/></svg>', 'Zero logística no evento', 'Dose individual — sem corte, sem preparação, sem imprevistos'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>', 'Uma sobremesa que fica na memória', 'Produto artesanal com história — os convidados percebem a diferença'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22V12"/><path d="M12 12C12 12 7 10 7 5a5 5 0 0 1 5-5 5 5 0 0 1 5 5c0 5-5 7-5 7z"/><path d="M9 15c-2 1-4 0-4 0s.5-3 2.5-4"/><path d="M15 15c2 1 4 0 4 0s-.5-3-2.5-4"/></svg>', 'Consistência em qualquer volume', 'Quatro sabores com qualidade igual em qualquer quantidade')], 'distribuidor': [('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>', 'Produto com procura crescente', 'Cada vez mais solicitado no canal HORECA em todo o país'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v2m0 8v2m-4-6h8"/></svg>', 'Condições competitivas por volume', 'Margem de distribuição atrativa, MOQ negociável por região'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6"/><line x1="8" y1="2" x2="8" y2="18"/><line x1="16" y1="6" x2="16" y2="22"/></svg>', 'Regiões com potencial por explorar', 'Zonas sem cobertura e clientela identificada à vossa espera'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10z"/><path d="M2 21c0-3 1.85-5.36 5.08-6C9.5 14.52 12 13 13 12"/></svg>', 'Marca com 40 anos e terceira geração', 'Reconhecimento nacional, história real, produto diferenciador')], 'gourmet': [('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="13" x2="15" y2="13"/><line x1="9" y1="17" x2="15" y2="17"/></svg>', 'Uma narrativa que se conta em duas frases', '40 anos de receita intacta — os seus clientes vão querer saber mais'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10z"/><path d="M2 21c0-3 1.85-5.36 5.08-6C9.5 14.52 12 13 13 12"/></svg>', 'Origem verificável, ingredientes simples', 'Produção familiar portuguesa — transparência total'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M8 6l4-4 4 4"/><path d="M8 18l4 4 4-4"/><path d="M6 8l-4 4 4 4"/><path d="M18 8l4 4-4 4"/><path d="M9 9l3-3 3 3"/><path d="M9 15l3 3 3-3"/><path d="M15 9l3 3-3 3"/><path d="M9 9l-3 3 3 3"/></svg>', 'Fácil de gerir em prateleira', 'Congelado individual — sem risco de validade, sem perdas'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>', 'Posicionamento premium acessível', 'Quatro sabores com valor percebido elevado')], 'cafe': [('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M17 8h1a4 4 0 0 1 0 8h-1"/><path d="M3 8h14v9a4 4 0 0 1-4 4H7a4 4 0 0 1-4-4Z"/><line x1="6" y1="2" x2="6" y2="4"/><line x1="10" y1="2" x2="10" y2="4"/><line x1="14" y1="2" x2="14" y2="4"/></svg>', 'O acompanhamento natural do café', 'Pão de ló artesanal e café de especialidade — uma combinação com alma'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v2m0 8v2m-4-6h8"/></svg>', 'Ticket médio mais elevado', 'Produto de impulso com margem interessante ao balcão'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="13" x2="15" y2="13"/><line x1="9" y1="17" x2="15" y2="17"/></svg>', 'Uma história que o barista pode contar', 'Dois minutos de conversa que tornam o produto inesquecível'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M8 6l4-4 4 4"/><path d="M8 18l4 4 4-4"/><path d="M6 8l-4 4 4 4"/><path d="M18 8l4 4-4 4"/><path d="M9 9l3-3 3 3"/><path d="M9 15l3 3 3-3"/><path d="M15 9l3 3-3 3"/><path d="M9 9l-3 3 3 3"/></svg>', 'Dose individual, zero desperdício', 'Serve só o que precisa — sem bolos inteiros que ficam para trás')], 'cervejaria': [('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>', 'Do frio ao prato em minutos', 'Zero preparação, zero espera — perfeito para o ritmo de uma cervejaria'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M8 6l4-4 4 4"/><path d="M8 18l4 4 4-4"/><path d="M6 8l-4 4 4 4"/><path d="M18 8l4 4-4 4"/><path d="M9 9l3-3 3 3"/><path d="M9 15l3 3 3-3"/><path d="M15 9l3 3-3 3"/><path d="M9 9l-3 3 3 3"/></svg>', 'Custo fixo e previsível', 'Dose individual — sem surpresas no final do mês'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v2m0 8v2m-4-6h8"/></svg>', 'Margem superior ao habitual', 'PVP atrativo com rentabilidade melhor do que pudim ou mousse'), ('<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22V12"/><path d="M12 12C12 12 7 10 7 5a5 5 0 0 1 5-5 5 5 0 0 1 5 5c0 5-5 7-5 7z"/><path d="M9 15c-2 1-4 0-4 0s.5-3 2.5-4"/><path d="M15 15c2 1 4 0 4 0s-.5-3-2.5-4"/></svg>', 'Os clientes reconhecem e pedem', 'Uma sobremesa portuguesa de sempre — sem precisar de ser explicada')]}

    def bens_html(tipo):
        rows = ""
        for svg_str, title, desc in BENS.get(tipo, BENS["restaurante"]):
            rows += (
                f'<tr><td style="padding:0 0 14px">'
                f'<table width="100%" cellpadding="0" cellspacing="0"><tr>'
                f'<td width="52" style="vertical-align:top;padding-right:14px">'
                f'<div style="width:44px;height:44px;background:#FDF6EF;border:1px solid #E8D5B8;'
                f'border-radius:8px;text-align:center;padding-top:10px;display:inline-block">'
                f'{svg_str}</div></td>'
    ...
    # ── Bloco central por número de email ─────────────────────────
    if email_num == 1:
        ...
        central = (
            ...
            '<p style="margin:4px 0;font-size:12px;color:#2C1A0A">&#10003;&nbsp; Passou a ser uma das sobremesas mais pedidas da carta</p>'
            '<p style="margin:4px 0;font-size:12px;color:#2C1A0A">&#10003;&nbsp; Desperdício em sobremesas: praticamente zero</p>'
            '<p style="margin:4px 0;font-size:12px;color:#2C1A0A">&#10003;&nbsp; <em>&ldquo;É o produto mais fácil que temos. Sai do frio, vai ao prato.&rdquo;</em></p>'
            '</div>'
        )
    elif email_num == 3:
        central = (
        ...
        central = (
            '<div style="background:#FDF6EF;border-radius:10px;padding:22px;text-align:center">'
            '<svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#C49A3C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="display:block;margin:0 auto">'
            '<polyline points="20 12 20 22 4 22 4 12"/><rect x="2" y="7" width="20" height="5"/>'
            '<line x1="12" y1="22" x2="12" y2="7"/>'
            '<path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"/>'
            '<path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"/>'
            '</svg>'
            '<p style="margin:12px 0 4px;font-size:15px;font-weight:700;color:#4a2417;font-family:Georgia,serif">Amostras sem compromisso</p>'
            '<p style="margin:0;font-size:12px;color:#6B5744">Original &nbsp;·&nbsp; Chocolate &nbsp;·&nbsp; Canela &nbsp;·&nbsp; Café</p>'
            '<p style="margin:12px 0 0;font-size:12px;color:#2C1A0A;line-height:1.7">Sem reuniões formais, sem apresentações longas.<br>Só o produto — para a vossa equipa provar e julgar com toda a liberdade.</p>'
            '</div>'
        )

    # ── Bloco de história (diferente por email) ───────────────────
...
<table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;width:100%">
  ...
  <tr><td style="background:#4a2417;border-radius:16px 16px 0 0;padding:22px 36px 16px;text-align:center">
    <img src="{LOGO}" alt="Ti\'Piedade" style="height:54px;display:block;margin:0 auto;border:0">
